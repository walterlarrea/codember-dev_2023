const _BD = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
const registros = _BD.split('\n')

const allUsers = registros.map((reg) => {
    const [id, username, email, age, location] = reg.split(',')
...
const hackedUsers = allUsers.map(user => {
    return {... user,
        valid: (/^[a-z0-9]+$/i.test(user.id) &&
    /^[a-z0-9]+$/i.test(user.username) &&
    /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(user.email) &&
    /^[a-z0-9]*$/i.test(user.age) &&
    /^[a-z0-9]*$/i.test(user.location))
}
})

const hiddenMessage = hackedUsers.reduce((acc, user, i) => {
    return user.valid ? acc : acc + user.username[0] || ''
}, '')

console.log(hiddenMessage) // FALLA