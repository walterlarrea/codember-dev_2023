...
for (let i = 0; i < 500; i++) {
    ...
    const [minMax, letra, clave] = element.split(" ")
    const [min, max] = minMax.split("-")
    
    let regex = new RegExp(letra[0], 'g')
    const coincidencias = clave.match(regex)

    if (!coincidencias || coincidencias?.length < min || coincidencias?.length > max) {
        erroneas.push([min, max, letra, clave])
    }
}

console.log("Clave de acceso super usuario: ", erroneas[12][3]) // Muestro la clave para conseguir permiso de super usuario
console.log("Clave del desafío:             ", erroneas[43][3]) // Muestro la clave solicitada por el desafío